...
const (
	...
	// caller sees a bare "await Pi native supervisor readiness: EOF" instead of
...
func awaitProcessTreeReady(launch *processTreeCommand) error {
	...
	if failure, ok := strings.CutPrefix(strings.TrimSpace(line), turnSupervisorFailure); ok {
		return fmt.Errorf("pi native supervisor failed before readiness: %s", failure)
	}
...
func parseTurnSupervisorLivenessReady(line string) (int, error) {
	...
	if failure, failed := strings.CutPrefix(text, turnSupervisorFailure); failed {
		return 0, fmt.Errorf("pi liveness failed before readiness: %s", failure)
	}
...
// pair credential-free tests use, or a standalone identity claimed here.
...
func runTurnSupervisorNative(
	...
	if publishCompletion {
		defer func() {
			...
			if livenessProtocol {
				_, _ = io.WriteString(readyOutput, turnSupervisorDoneLine)
			}
		}()
	}
