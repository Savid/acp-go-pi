/**
 * acp-go-pi native-shell PATH extension.
 *
 * pi prepends PI_CODING_AGENT_DIR/bin when it creates the built-in bash tool's
 * environment. Restore the session's ordered extraPathDirs after that native
 * rewrite so they stay first for both agent tool calls and user bash commands.
 */
...
const extraPathDirs = (process.env.ACP_GO_PI_INTERNAL_EXTRA_PATH_DIRS ?? "")
	.split(delimiter)
	.filter(Boolean);
