...
func TestTerminalEntitiesRefuseEveryCarriedDifference(t *testing.T) {
	...
	t.Run("action", func(t *testing.T) {
		...
		for name, later := range map[string]ActionUpdate{
			...
			"changed blocking":   {ActionID: "req-1", State: ActionAccepted, BlocksForeground: new(true)},
		} {
			t.Run(name, func(t *testing.T) {
				t.Parallel()
...
// TestProofClassIsClosedAtOne pins that nothing outside the single proof class is
// a class at all.
